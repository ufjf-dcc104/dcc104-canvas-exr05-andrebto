...
Sprite.prototype.colidiuCircleBox = function (alvo) {

  var closest = [];
  if(this.x < alvo.x - alvo.width/2) 
    closest.push(alvo.x - alvo.width/2);
  else if(this.x > alvo.x + alvo.width/2)
    closest.push(alvo.x + alvo.width/2);
  else
    closest.push(this.x);

  if(this.y < alvo.y - alvo.height/2) 
    closest.push(alvo.y);
  else if(this.y > alvo.y + alvo.height/2)
    closest.push(alvo.y + alvo.height/2);
  else
    closest.push(this.y);

  
  var dX = closest[0] - this.x;
  var dY = closest[1] - this.y;
  var dist_square = dX * dX + dY * dY;
  if(dist_square < this.radius * this.radius)
    return true;
  return false;
}
